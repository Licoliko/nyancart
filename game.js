...
function drawIntroRivalPortraits(rivals){
  ...
  rivals.forEach((racer,index)=>{
    const face=canvases[index],frame=racer.frames?.[10]||racer.frames?.[9]||racer.frames?.[11];
    if(!face||!frame)return;
    const g=face.getContext('2d'),cropWidth=frame.sw*.58,cropHeight=frame.sh*.58,sourceX=frame.sx+(frame.sw-cropWidth)/2,sourceY=frame.sy+frame.sh*.015;
    g.clearRect(0,0,face.width,face.height);g.imageSmoothingEnabled=true;g.imageSmoothingQuality='high';g.drawImage(frame.image,sourceX,sourceY,cropWidth,cropHeight,0,0,face.width,face.height);
    face.dataset.painted='race-sprite-front-face';face.previousElementSibling?.classList.add('fallback-hidden');
  });
}
...
function drawCourseShowcaseMap(now=performance.now()){
  const c=$('courseSelectMap');if(!c||state.mode!=='course'||!activeCourse)return;const m=c.getContext('2d'),w=c.width,h=c.height,pad=24,theme=activeCourse.theme||courseData[0].theme,zones=biomeVolumeZones(state.selectedCourse),legendHeight=zones.length>4?67:49,mapHeight=h-legendHeight,reveal=clamp((now-courseCarousel.revealStart)/620,0,1),travel=wrap01((now-courseCarousel.revealStart)*.000055+.01),point=(progress,offset=0)=>{const sample=trackSample(progress),px=sample.x-sample.ty*offset,py=sample.y+sample.tx*offset;return{x:pad+px*(w-pad*2),y:pad*.58+py*(mapHeight-pad*.72)}};
...
function drawPodiumPortraitFallback(canvas,racer,rank){if(racer===miaNpc)return drawMiaPodiumSpriteFallback(canvas,rank);return new Promise((resolve,reject)=>{const image=new Image();image.onload=()=>{const c=canvas.getContext('2d'),scale=Math.min(canvas.width/image.naturalWidth*(rank===1?.92:.88),(canvas.height-12)/image.naturalHeight*.92),w=image.naturalWidth*scale,h=image.naturalHeight*scale;c.clearRect(0,0,canvas.width,canvas.height);c.drawImage(image,(canvas.width-w)/2,canvas.height-h-3,w,h);canvas.dataset.painted='portrait-fallback';canvas.dataset.podiumSource='portrait-fallback';canvas.dataset.podiumHeadroom=String(Math.max(0,Math.round(canvas.height-h-3)));resolve(true)};image.onerror=reject;image.src=racer.portrait})}
function drawResultFaceSprite(canvas,racer){
  if(!canvas||!racer)return;
  const rawFrame=racer.frames?.[PODIUM_FRONT_FRAME_INDEX]||racer.frames?.[3]||racer.frames?.[0],frame=podiumFaceBounds(rawFrame),c=canvas.getContext('2d');
  c.clearRect(0,0,canvas.width,canvas.height);
  if(!frame)return;
  ...
  c.restore();canvas.dataset.faceSource='game-sprite-front-face-v3-contain';canvas.dataset.faceRacer=racer.slug;canvas.dataset.faceFrame=String(PODIUM_FRONT_FRAME_INDEX);canvas.dataset.faceFit='alpha-upper-body-contain';canvas.dataset.faceHeadroom=String(Math.max(0,Math.round(drawY)));
}
...
function drawBiomeVolume(layer){
  ...
  entries.sort((a,b)=>b.rel-a.rel);let drawn=0;for(const entry of entries){const{cluster,prop,rel,p}=entry,frame=frames[prop.frame%frames.length];if(!frame)continue;const zone=zones[cluster.zoneIndex%zones.length],baseHeight=(layer==='far'?1370:layer==='mid'?940:700)*(index===6?1.08:1),cap=layer==='far'?.6:layer==='mid'?.54:.46,height=Math.max(10,Math.min(innerHeight*cap,baseHeight*p.scale*prop.scale*(.7+performance*.3))),rawX=p.cx+prop.side*p.half*prop.offset,x=rel<58?clamp(rawX,-height*.3,innerWidth+height*.3):rawX,width=height*frame.sw/Math.max(1,frame.sh);if(x<-width||x>innerWidth+width)continue;const groundY=p.y+Math.max(2,height*.055),fog=sceneryFogAmount(rel),fadeNear=layer==='far'?118:layer==='mid'?34:9,fadeStart=layer==='far'?365:layer==='mid'?425:495,alpha=clamp(rel/fadeNear,0,1)*distanceFadeAlpha(rel,fadeStart,815)*(1-fog*(layer==='far'?.34:.16));if(alpha<.035)continue;
    if(layer!=='far'){ctx.save();ctx.globalAlpha=alpha*(layer==='near'?.3:.2);ctx.fillStyle=colorAlpha(profile.shade,layer==='near'?.9:.72);ctx.beginPath();ctx.ellipse(x,groundY,Math.max(4,width*.32),Math.max(1.5,height*.052),0,0,Math.PI*2);ctx.fill();ctx.restore()}drawThroughTunnelPortal(entry.z,()=>drawFrameHeightFiltered(frame,x,groundY,height,alpha,0,sceneryFilter(rel)));drawn++
  }
...
function drawCourseRouteOcclusionVolume(){
  ...
  for(const entry of entries){const{p,median,rel,slot}=entry,left=routeCorridorPoint(p,median.left),right=routeCorridorPoint(p,median.right),innerLeft=left.cx+left.half,innerRight=right.cx-right.half,gap=Math.max(0,innerRight-innerLeft);if(gap<5)continue;const center=(innerLeft+innerRight)*.5,groundY=(left.y+right.y)*.5+Math.max(1,p.half*.018),alpha=distanceFadeAlpha(rel,540,805)*(1-sceneryFogAmount(rel)*.11);if(alpha<.04)continue;
    ...
    const offsets=rich?[-.2,.2]:[0];for(let cluster=0;cluster<offsets.length;cluster++){const frameIndex=profile.frames[(Math.abs(slot)+cluster*2)%profile.frames.length],frame=frames[frameIndex];if(!frame)continue;const ratio=frame.sw/Math.max(1,frame.sh),maxWidth=gap*profile.fill/(rich?1.54:1),height=Math.max(8,Math.min(innerHeight*(jungle ? .7 : .56),maxWidth/Math.max(.28,ratio),p.scale*profile.base*(cluster ? .86 : 1))),width=height*ratio,x=center+gap*offsets[cluster];if(width<4)continue;ctx.save();ctx.globalAlpha=alpha*.3;ctx.fillStyle='rgba(3,6,18,.92)';ctx.beginPath();ctx.ellipse(x,groundY,Math.min(gap*.24,width*.48),Math.max(1.5,height*.05),0,0,Math.PI*2);ctx.fill();ctx.restore();drawFrameHeightFiltered(frame,x,groundY,height,alpha,0,sceneryFilter(rel));drawn++}
  }
