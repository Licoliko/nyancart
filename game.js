...
const COSTUME_CATALOG=Object.freeze({
  'aruka-sham':Object.freeze([
    ...
    Object.freeze({id:'grid-queen',name:'NYAN GRID QUEEN',kicker:'GRAND PRIX AMBASSADOR COLLECTION',description:'パールホワイトとネイビーへシアン、ホットピンク、ローズゴールドを走らせたグランプリ・アンバサダー仕様。選択画面ではチェッカーフラッグを掲げ、レース中は両手でハンドルを握ります。性能は通常衣装と同じです。',unlockLabel:'アルカで、いずれかのコースを3位以内で完走',select:'assets/costumes/aruka-sham/grid-queen/select.webp',sprite:'assets/costumes/aruka-sham/grid-queen/sprite.webp',source:'GPT IMAGE 2.0'})
  ]),
  'kurone-night':Object.freeze([
    ...
    Object.freeze({id:'lunar-grid-queen',name:'LUNAR GRID QUEEN',kicker:'NYAN GRAND PRIX GRID COLLECTION',description:'ミッドナイトネイビーと紫、銀のチェッカーを重ねた月光グリッドクイーン仕様。性能は通常衣装と同じです。',unlockLabel:'クロネで、いずれかのコースを3位以内で完走',select:'assets/costumes/kurone-night/lunar-grid-queen/select.webp',sprite:'assets/costumes/kurone-night/lunar-grid-queen/sprite.webp',source:'GPT IMAGE 2.0'})
  ]),
  'kohaku-taiga':Object.freeze([
    ...
    Object.freeze({id:'tiger-grid-queen',name:'TIGER GRID QUEEN',kicker:'NYAN GRAND PRIX GRID COLLECTION',description:'クリームとオレンジ、黒のチェッカーを合わせた猛虎グリッドクイーン仕様。性能は通常衣装と同じです。',unlockLabel:'コハクで、いずれかのコースを3位以内で完走',select:'assets/costumes/kohaku-taiga/tiger-grid-queen/select.webp',sprite:'assets/costumes/kohaku-taiga/tiger-grid-queen/sprite.webp',source:'GPT IMAGE 2.0'})
  ]),
  'ghost-rex':Object.freeze([
    ...
    Object.freeze({id:'phantom-grid-queen',name:'PHANTOM GRID QUEEN',kicker:'NYAN GRAND PRIX GRID COLLECTION',description:'深いプラムと黒、アンティークシルバーに幽光を添えた幻影グリッドクイーン仕様。性能は通常衣装と同じです。',unlockLabel:'ゴースで、いずれかのコースを3位以内で完走',select:'assets/costumes/ghost-rex/phantom-grid-queen/select.webp',sprite:'assets/costumes/ghost-rex/phantom-grid-queen/sprite.webp',source:'GPT IMAGE 2.0'})
  ]),
  'nerine-korat':Object.freeze([
    ...
    Object.freeze({id:'aqua-grid-queen',name:'AQUA GRID QUEEN',kicker:'NYAN GRAND PRIX GRID COLLECTION',description:'パールホワイトとアクア、珊瑚色のチェッカーで仕立てた海宮グリッドクイーン仕様。性能は通常衣装と同じです。',unlockLabel:'ネリネで、いずれかのコースを3位以内で完走',select:'assets/costumes/nerine-korat/aqua-grid-queen/select.webp',sprite:'assets/costumes/nerine-korat/aqua-grid-queen/sprite.webp',source:'GPT IMAGE 2.0'})
  ]),
  'fumika-scotty':Object.freeze([
    ...
    Object.freeze({id:'airmail-grid-queen',name:'AIRMAIL GRID QUEEN',kicker:'NYAN GRAND PRIX GRID COLLECTION',description:'アイボリーとコーラル、ネイビーに航空便チェッカーを重ねたグリッドクイーン仕様。性能は通常衣装と同じです。',unlockLabel:'フミカで、いずれかのコースを3位以内で完走',select:'assets/costumes/fumika-scotty/airmail-grid-queen/select.webp',sprite:'assets/costumes/fumika-scotty/airmail-grid-queen/sprite.webp',source:'GPT IMAGE 2.0'})
  ]),
  'bell-savanna':Object.freeze([
    ...
    Object.freeze({id:'savanna-grid-queen',name:'SAVANNA GRID QUEEN',kicker:'NYAN GRAND PRIX GRID COLLECTION',description:'ゴールドと深紅、黒のチェッカーで仕立てたサバンナ王者のグリッドクイーン仕様。性能は通常衣装と同じです。',unlockLabel:'ベルで、いずれかのコースを3位以内で完走',select:'assets/costumes/bell-savanna/savanna-grid-queen/select.webp',sprite:'assets/costumes/bell-savanna/savanna-grid-queen/sprite.webp',source:'GPT IMAGE 2.0'})
  ]),
...
function playUnlockAnimation(racer,cost){
  ...
  fx.dataset.unlockType='racer';delete fx.dataset.unlockTheme;delete fx.dataset.unlockPhase;fx.style.setProperty('--unlock-color',racer.color||'#48dcff');$('unlockEyebrow').textContent='NEW RACER UNLOCKED';$('unlockHero').src=racerSelectHero(racer);$('unlockHero').alt=racer.name;$('unlockName').textContent=racer.name;$('unlockCopy').textContent=`${cost} COINSで専用セットを開放！`;
...
function playCostumeUnlockAnimation(racer,costume){
  const fx=$('unlockFx');if(!fx||!racer||!costume)return 0;const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches,gridQueen=racer.slug==='aruka-sham'&&costume.id==='grid-queen',duration=reduced?(gridQueen?1150:980):(gridQueen?3900:2700);
  fx.dataset.unlockType='costume';if(gridQueen){fx.dataset.unlockTheme='grid-queen';fx.dataset.unlockPhase='lights'}else{delete fx.dataset.unlockTheme;delete fx.dataset.unlockPhase}fx.style.setProperty('--unlock-color',gridQueen?'#54edff':racer.color||'#ff5dc8');$('unlockEyebrow').textContent=gridQueen?'GRID LIGHTS READY':'NEW OUTFIT ACQUIRED';$('unlockHero').src=costume.select;$('unlockHero').alt=`${racer.name} ${costume.name}`;$('unlockName').textContent=costume.name;$('unlockCopy').textContent=gridQueen?'グリッドを照らす、アルカのチェッカーフラッグ！':`${racer.name}の新しいレーシングスタイルを解放！`;
  playCostumeUnlockAnimation.stageTimers?.forEach(clearTimeout);playCostumeUnlockAnimation.stageTimers=[];makeConfetti($('unlockConfetti'),gridQueen?72:58);fx.classList.remove('hidden','show');void fx.offsetWidth;fx.classList.add('show');
  if(gridQueen&&!reduced){playSfx('uiMove',{intensity:.34});playCostumeUnlockAnimation.stageTimers.push(setTimeout(()=>{fx.dataset.unlockPhase='hero';$('unlockEyebrow').textContent='GRID QUEEN ARRIVAL';playSfx('resultReward',{intensity:.82});navigator.vibrate?.([48,35,78])},1120),setTimeout(()=>{fx.dataset.unlockPhase='flag';$('unlockEyebrow').textContent='CHECKERED FLAG!';playSfx('resultStamp',{intensity:1.12});navigator.vibrate?.([85,38,135])},2240))}else{playSfx('resultStamp',{intensity:1.05});navigator.vibrate?.([55,35,110])}
  canvas.dataset.costumeUnlockAnimation=`${racer.slug}:${costume.id}`;canvas.dataset.costumeUnlockFx=gridQueen?'grid-lights-hero-flag-v1':'outfit-card-orbit-scan-v1';canvas.dataset.costumeUnlockSequence=gridQueen?'lights>aruka-rise>checker-flag':'card>hero>ribbon';
  clearTimeout(playCostumeUnlockAnimation.t);playCostumeUnlockAnimation.t=setTimeout(()=>{fx.classList.add('hidden');fx.classList.remove('show');delete fx.dataset.unlockPhase;$('unlockConfetti').innerHTML=''},duration);return duration
}
...
function renderWardrobe(){
  ...
  const replay=$('wardrobeReplayUnlock'),replayable=unlocked&&costume.id!=='standard';replay.disabled=!replayable;replay.dataset.costume=costume.id;replay.dataset.replayModel='selected-costume-unlock-replay-v1';replay.querySelector('span').textContent=replayable?'WATCH UNLOCK AGAIN':costume.id==='standard'?'SELECT AN OUTFIT':'LOCKED';replay.querySelector('small').textContent=replayable?(costume.id==='grid-queen'?'グリッドライト演出をもう一度見る':'解放演出をもう一度見る'):costume.id==='standard'?'別衣装を選択してください':'解放後に再生できます';replay.setAttribute('aria-label',replayable?`${costume.name}の解放演出をもう一度見る`:replay.querySelector('small').textContent);
  ...
  $('wardrobe').dataset.racer=racer.slug;$('wardrobe').dataset.preview=costume.id;$('wardrobe').dataset.equipped=activeCostumeId(racer);$('wardrobe').dataset.unlocked=String(unlocked);$('wardrobe').dataset.unlockModel='any-course-top3-v1';$('wardrobe').dataset.unlockReplay=String(replayable);ensureWardrobeRotationPreview()
}
...
function replayWardrobeUnlockAnimation(){
  const racer=racers[state.selected],costume=selectedWardrobeCostume(),button=$('wardrobeReplayUnlock');if(!racer||!costume||costume.id==='standard'||!isCostumeUnlocked(racer,costume)){playSfx('uiBack',{intensity:.34});return 0}const duration=playCostumeUnlockAnimation(racer,costume);canvas.dataset.costumeUnlockReplay=`${racer.slug}:${costume.id}`;$('wardrobe').dataset.lastUnlockReplay=costume.id;uiMotionDirector.reward(button);return duration
}
