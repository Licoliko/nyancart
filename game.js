...
const courseRaceMusicFiles=Object.freeze({phantom:'assets/audio/phantom_gear_parade.mp3'});
...
function playRaceMusic(){if(raceBgm)raceBgm.pause();const courseKey=courseScenerySlugs[state.selectedCourse],override=courseRaceMusicFiles[courseKey],source=override||defaultRaceMusicFiles[raceMusicIndex++%defaultRaceMusicFiles.length];raceBgm=raceMusicByFile.get(source)||raceMusic[0];raceBgm.currentTime=0;musicError='';canvas.dataset.bgmCourse=courseKey||'debug';canvas.dataset.bgmTrack=source.split('/').pop();canvas.dataset.bgmMode=override?'course-exclusive':'grand-prix-rotation';const playing=raceBgm.play();syncMusicButton();playing?.then(syncMusicButton).catch(error=>{musicError=error?.name||'play-failed';syncMusicButton()})}
...
function drawResultFaceSprite(canvas,racer){
  if(!canvas||!racer)return;
  const rawFrame=racer.frames?.[PODIUM_FRONT_FRAME_INDEX]||racer.frames?.[3]||racer.frames?.[0],frame=podiumVisibleBounds(rawFrame),c=canvas.getContext('2d');
  c.clearRect(0,0,canvas.width,canvas.height);
  if(!frame)return;
  const cropSize=Math.min(frame.sw*.56,frame.sh*.52),cropX=frame.sx+(frame.sw-cropSize)/2,cropY=frame.sy+frame.sh*.005,destinationInset=3,destinationSize=canvas.width-destinationInset*2;
  c.save();
  ...
  c.drawImage(frame.image,cropX,cropY,cropSize,cropSize,destinationInset,destinationInset,destinationSize,destinationSize);
  c.restore();
  c.save();
  c.strokeStyle='#fff';c.lineWidth=4;c.beginPath();c.arc(canvas.width/2,canvas.height/2,canvas.width*.45,0,Math.PI*2);c.stroke();
  c.restore();canvas.dataset.faceSource='game-sprite-front-face-v2';canvas.dataset.faceRacer=racer.slug;canvas.dataset.faceFrame=String(PODIUM_FRONT_FRAME_INDEX);
}
