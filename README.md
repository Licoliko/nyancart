...
レーサーセット選択の「OUTFIT LOCKER」から衣装を変更できます。アルカの「CUP CHAMPION」に加え、クロネ「MOONLIGHT ACE」、コハク「SUNSET RALLY」、ゴース「PHANTOM GALA」、ネリネ「AQUA PARADE」をGPT Image 2.0製の別衣装として収録しています。ロッカーでは7列×2段の全14コマを正しい周回順へ並べ替えた自動回転プレビューで方向を確認できます。「INSPECT」では自動回転を止め、前後1コマ送り、14コマ直接選択、問題コマのマーク／解除が可能です。問題マークはレーサーと衣装ごとにブラウザへ保存され、「CSV REPORT」でキャラ名・衣装名・方向・元セル番号を一覧化し、「PNG CONTACT SHEET」で該当セルの画像一覧を書き出せます。衣装は選択画像とレース用14方向スプライトを同時に切り替え、性能・カート・固有能力は通常衣装から変化しません。装備状態と解放状態もブラウザへ保存されます。
