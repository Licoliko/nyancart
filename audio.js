(function(){
  ...
  const BIOME_AMBIENCE_PROFILES=Object.freeze({
    sweets:{label:'SUGAR CHIME BREEZE',wave:'sine',toneHz:659,noiseHz:1850,Q:2.4,level:.052,noise:.34,tone:.18,zonePitch:[1,1.122,.944,1.26]},
    steam:{label:'BOILER DISTRICT AIR',wave:'sawtooth',toneHz:82,noiseHz:420,Q:.82,level:.068,noise:.58,tone:.16,zonePitch:[1,.89,.79,1.06]},
    neon:{label:'SKYLINE ELECTRIC HUM',wave:'square',toneHz:110,noiseHz:2380,Q:3.2,level:.048,noise:.28,tone:.13,zonePitch:[1,1.19,1.335,1.06]},
    rain:{label:'FOREST RAIN BED',wave:'sine',toneHz:147,noiseHz:3200,Q:.62,level:.072,noise:.7,tone:.08,zonePitch:[1,.944,.84,1.06]},
    royal:{label:'PALACE GARDEN CHIMES',wave:'triangle',toneHz:523,noiseHz:1280,Q:2.1,level:.05,noise:.26,tone:.18,zonePitch:[1,1.122,1.26,.944]},
    aurora:{label:'CRYSTAL GLACIER WIND',wave:'sine',toneHz:622,noiseHz:2700,Q:1.25,level:.064,noise:.48,tone:.2,zonePitch:[.89,1.122,.944,1.335]},
    jungle:{label:'EMERALD CANOPY LIFE',wave:'sine',toneHz:196,noiseHz:4100,Q:4.2,level:.068,noise:.55,tone:.09,zonePitch:[1,1.122,1.26,.944,.84,1.19,.89,1.335]},
    sakura:{label:'ONSEN BLOSSOM BREEZE',wave:'sine',toneHz:440,noiseHz:1650,Q:1.6,level:.052,noise:.34,tone:.15,zonePitch:[1,1.122,.944,1.26]},
    coral:{label:'AQUA PALACE CURRENT',wave:'sine',toneHz:247,noiseHz:760,Q:1.05,level:.062,noise:.52,tone:.12,zonePitch:[1,1.19,1.335,.89]},
    phantom:{label:'CARNIVAL GHOST AIR',wave:'triangle',toneHz:92,noiseHz:2100,Q:3.4,level:.06,noise:.43,tone:.14,zonePitch:[1,.79,1.26,.89]},
    lunatic:{label:'ORBITAL COLONY FIELD',wave:'triangle',toneHz:55,noiseHz:340,Q:1.8,level:.056,noise:.31,tone:.18,zonePitch:[1,1.335,.944,1.19]}
  });

  class NyanAudioSystem{
    constructor(){
      this.context=null;this.master=null;this.compressor=null;this.engineBus=null;this.rivalBus=null;this.sfxBus=null;this.ambienceBus=null;this.raceTone=null;this.tunnelSend=null;this.tunnelConvolver=null;this.tunnelReturn=null;this.engine=null;
      this.machineSlug='';this.machineStats=null;this.running=false;this.mix={master:.8,effects:.72,muted:false};
      this.lastPlayed=new Map();this.previewTimer=0;this.noiseBuffer=null;this.rivalVoices=new Map();this.biomeAmbienceVoices=[];this.biomeAmbienceActive=0;this.biomeAmbienceKey='none';this.biomeAmbienceCourse='none';this.biomeAmbienceZone='none';this.biomeAmbienceLabel='none';this.biomeAmbienceSwitchAt=0;this.biomeAmbienceDuration=1800;this.biomeAmbienceTarget=0;this.maxRivalVoices=3;this.dopplerEnabled=true;this.tunnelReverbEnabled=true;this.tunnelMix=0;this.surfaceAudio='dry';this.surfaceAudioLevel=0;this.surfacePan=0;this.enginePan=0;this.pickupRespawnPan=0;this.pickupRespawnCount=0;this.lastSignatureCue='none';this.signatureCueCount=0;this.lastPlayerSignatureCue='none';this.playerSignatureCueCount=0;this.supported=!!(window.AudioContext||window.webkitAudioContext);
      ...
      this.master=context.createGain();this.compressor=context.createDynamicsCompressor();this.engineBus=context.createGain();this.rivalBus=context.createGain();this.sfxBus=context.createGain();this.ambienceBus=context.createGain();this.raceTone=context.createBiquadFilter();this.tunnelSend=context.createGain();this.tunnelConvolver=context.createConvolver();this.tunnelReturn=context.createGain();
      this.engineBus.gain.value=.82;this.rivalBus.gain.value=.38;this.sfxBus.gain.value=.78;this.ambienceBus.gain.value=.24;this.master.gain.value=0;
    ...
    createBiomeAmbienceVoice(){
      const context=this.ensure();if(!context||!this.ambienceBus)return null;const now=context.currentTime,output=context.createGain(),noise=context.createBufferSource(),noiseFilter=context.createBiquadFilter(),noiseGain=context.createGain(),tone=context.createOscillator(),toneGain=context.createGain();
      output.gain.value=.0001;noise.buffer=this.noiseBuffer;noise.loop=true;noiseFilter.type='bandpass';noiseFilter.frequency.value=1200;noiseFilter.Q.value=1;noiseGain.gain.value=.3;tone.type='sine';tone.frequency.value=220;toneGain.gain.value=.08;
      noise.connect(noiseFilter);noiseFilter.connect(noiseGain);noiseGain.connect(output);tone.connect(toneGain);toneGain.connect(output);output.connect(this.ambienceBus);noise.start(now);tone.start(now);
      return{output,noise,noiseFilter,noiseGain,tone,toneGain}
    }
    ensureBiomeAmbienceVoices(){while(this.biomeAmbienceVoices.length<2){const voice=this.createBiomeAmbienceVoice();if(!voice)break;this.biomeAmbienceVoices.push(voice)}return this.biomeAmbienceVoices.length===2}
    configureBiomeAmbienceVoice(voice,profile,zoneIndex=0){
      if(!voice||!this.context)return;const now=this.context.currentTime,pitches=profile.zonePitch||[1],pitch=pitches[Math.abs(zoneIndex)%pitches.length]||1,variation=1+((Math.abs(zoneIndex)%3)-1)*.08;
      voice.tone.type=profile.wave||'sine';voice.tone.frequency.setTargetAtTime(Math.max(24,profile.toneHz*pitch),now,.12);voice.toneGain.gain.setTargetAtTime(Math.max(.012,profile.tone*variation),now,.14);voice.noiseFilter.frequency.setTargetAtTime(Math.max(70,profile.noiseHz*pitch),now,.12);voice.noiseFilter.Q.setTargetAtTime(Math.max(.35,profile.Q),now,.12);voice.noiseGain.gain.setTargetAtTime(Math.max(.04,profile.noise/variation),now,.14)
    }
    updateBiomeAmbience({course='sweets',zone='zone',zoneIndex=0,running=false,paused=false,tunnelMix=0}={}){
      const profile=BIOME_AMBIENCE_PROFILES[course]||BIOME_AMBIENCE_PROFILES.sweets,key=`${course}:${zone}`,audible=!!running&&!paused&&!this.mix.muted&&this.mix.effects>0,level=audible?profile.level*(1-Math.max(0,Math.min(1,Number(tunnelMix)||0))*.68):.0001;
      this.biomeAmbienceCourse=course;this.biomeAmbienceZone=zone;this.biomeAmbienceLabel=profile.label;this.biomeAmbienceTarget=level;if(!this.context)return false;if(!this.ensureBiomeAmbienceVoices())return false;
      const now=this.context.currentTime;if(key!==this.biomeAmbienceKey){const previous=this.biomeAmbienceVoices[this.biomeAmbienceActive],nextIndex=this.biomeAmbienceKey==='none'?this.biomeAmbienceActive:1-this.biomeAmbienceActive,next=this.biomeAmbienceVoices[nextIndex];this.configureBiomeAmbienceVoice(next,profile,zoneIndex);for(const voice of this.biomeAmbienceVoices)voice.output.gain.cancelScheduledValues(now);if(previous&&previous!==next)previous.output.gain.setTargetAtTime(.0001,now,.46);next.output.gain.setValueAtTime(Math.max(.0001,next.output.gain.value),now);next.output.gain.setTargetAtTime(Math.max(.0001,level),now,.46);this.biomeAmbienceActive=nextIndex;this.biomeAmbienceKey=key;this.biomeAmbienceSwitchAt=performance.now()}else{const active=this.biomeAmbienceVoices[this.biomeAmbienceActive];active.output.gain.setTargetAtTime(Math.max(.0001,level),now,audible?.18:.12);this.biomeAmbienceVoices[1-this.biomeAmbienceActive]?.output.gain.setTargetAtTime(.0001,now,.2)}
      return true
    }
    getBiomeAmbienceDebug(){return{model:'dual-bus-zone-crossfade-v1',course:this.biomeAmbienceCourse,zone:this.biomeAmbienceZone,label:this.biomeAmbienceLabel,key:this.biomeAmbienceKey,mix:Math.max(0,Math.min(1,(performance.now()-this.biomeAmbienceSwitchAt)/this.biomeAmbienceDuration)),target:this.biomeAmbienceTarget}}
    ...
    stopEngine(){this.running=false;if(this.engine&&this.context)this.engine.output.gain.setTargetAtTime(.0001,this.context.currentTime,.09);this.stopRivals();this.updateBiomeAmbience({course:this.biomeAmbienceCourse,zone:this.biomeAmbienceZone,running:false});this.setTunnel(false)}
    updateEngine(data={}){
    ...
    noise({duration=.16,gain=.11,frequency=900,to=frequency,type='bandpass',delay=0,Q=1.2,pan=0}={}){
      const context=this.unlock();if(!context||!this.sfxBus)return;const start=context.currentTime+delay,end=start+duration,source=context.createBufferSource(),filter=context.createBiquadFilter(),amp=context.createGain(),panner=context.createStereoPanner?.()||null;source.buffer=this.noiseBuffer;filter.type=type;filter.Q.value=Q;filter.frequency.setValueAtTime(Math.max(40,frequency),start);filter.frequency.exponentialRampToValueAtTime(Math.max(40,to),end);amp.gain.setValueAtTime(.0001,start);amp.gain.exponentialRampToValueAtTime(Math.max(.0002,gain),start+.008);amp.gain.exponentialRampToValueAtTime(.0001,end);source.connect(filter);filter.connect(amp);if(panner){panner.pan.value=Math.max(-1,Math.min(1,Number(pan)||0));amp.connect(panner);panner.connect(this.sfxBus)}else amp.connect(this.sfxBus);source.onended=()=>{try{amp.disconnect()}catch{}try{panner?.disconnect()}catch{}};source.start(start);source.stop(end+.03)
    }
    ...
    play(name,{intensity=1,variant=0,cue='prism',role='world',pan=0}={}){
      ...
      switch(name){
        ...
        case'routeHazardWarn':if(!this.allowed(`${name}:${variant}`,190))return;if(variant===0){this.tone({frequency:188,to:152,duration:.09,gain:.085*k,type:'triangle',pan});this.noise({duration:.08,gain:.055*k,frequency:780,to:310,Q:1.8,pan});this.tone({frequency:218,to:172,duration:.1,gain:.075*k,delay:.12,type:'triangle',pan})}else{this.tone({frequency:820,to:1120,duration:.11,gain:.072*k,type:'sine',pan});this.tone({frequency:540,to:410,duration:.18,gain:.06*k,delay:.08,type:'square',pan});this.noise({duration:.22,gain:.043*k,frequency:1280,to:280,Q:2.2,pan})}break;
        case'routeHazardClose':if(!this.allowed(`${name}:${variant}`,230))return;if(variant===0){this.noise({duration:.32,gain:.18*k,frequency:930,to:95,Q:.72,pan});this.tone({frequency:118,to:42,duration:.3,gain:.145*k,type:'square',pan});this.tone({frequency:246,to:92,duration:.16,gain:.068*k,delay:.035,type:'triangle',pan})}else{this.noise({duration:.46,gain:.17*k,frequency:520,to:52,Q:.58,pan});this.tone({frequency:82,to:31,duration:.48,gain:.16*k,type:'sawtooth',pan});this.tone({frequency:760,to:290,duration:.19,gain:.075*k,delay:.06,type:'square',pan})}break;
        case'courseGimmickWarn':if(!this.allowed(`${name}:${variant}`,180))return;this.tone({frequency:520+variant*72,to:760+variant*86,duration:.11,gain:.055*k,type:variant%2?'triangle':'sine',pan});this.tone({frequency:740+variant*55,to:980+variant*82,duration:.1,gain:.038*k,delay:.1,type:'square',pan});break;
        case'courseGimmickActive':if(!this.allowed(`${name}:${variant}`,210))return;this.noise({duration:.2,gain:.075*k,frequency:1500+variant*180,to:240+variant*35,Q:1.1,pan});this.tone({frequency:180+variant*24,to:82+variant*12,duration:.24,gain:.08*k,type:variant%2?'sawtooth':'triangle',pan});break;
        case'courseGimmickHit':if(!this.allowed(`${name}:${variant}`,150))return;this.noise({duration:.24,gain:.13*k,frequency:1900+variant*120,to:130,Q:.7,pan});this.tone({frequency:150+variant*18,to:48+variant*7,duration:.22,gain:.11*k,type:'square',pan});break;
        ...
        case'finalLap':this.noise({duration:.32,gain:.045*k,frequency:480,to:2800,Q:1.4});[659,784,1047].forEach((frequency,index)=>this.tone({frequency,to:frequency*1.08,duration:.2,gain:.075*k,delay:index*.105,type:index===2?'square':'triangle'}));break;
        case'tutorialStep':if(!this.allowed(name,220))return;this.tone({frequency:620+variant*90,to:840+variant*120,duration:.09,gain:.052*k,type:'triangle'});this.tone({frequency:930+variant*110,to:1180+variant*130,duration:.12,gain:.038*k,delay:.055,type:'sine'});break;
        case'resultCount':if(!this.allowed(name,70))return;this.tone({frequency:780+variant*45,to:930+variant*55,duration:.045,gain:.035*k,type:'triangle'});break;
        case'resultStamp':this.noise({duration:.13,gain:.045*k,frequency:1400,to:320,Q:1.2});this.tone({frequency:520,to:1040,duration:.2,gain:.075*k,type:'square'});this.tone({frequency:880,to:1320,duration:.17,gain:.052*k,delay:.08,type:'triangle'});break;
        case'resultReward':this.tone({frequency:660+variant*80,to:990+variant*120,duration:.13,gain:.065*k,type:'triangle'});this.tone({frequency:990+variant*90,to:1480+variant*130,duration:.16,gain:.042*k,delay:.055,type:'sine'});break;
        case'loadReady':this.noise({duration:.18,gain:.025*k,frequency:620,to:2100,Q:1.6});[520,780,1040].forEach((frequency,index)=>this.tone({frequency,to:frequency*1.12,duration:.15,gain:.048*k,delay:index*.065,type:'triangle'}));break;
        ...
        case'miaBoneTell':if(!this.allowed(name,230))return;this.tone({frequency:930,to:1320,duration:.075,gain:.072*k,type:'triangle'});this.tone({frequency:1240,to:1710,duration:.09,gain:.056*k,delay:.085,type:'square'});this.noise({duration:.07,gain:.038*k,frequency:2700,to:1900,Q:2.8,delay:.04});break;
        case'miaCanTell':if(!this.allowed(name,230))return;for(let i=0;i<3;i++)this.tone({frequency:760+i*165,to:470+i*105,duration:.065,gain:(.06-i*.009)*k,delay:i*.062,type:i%2?'triangle':'square',detune:i%2?17:-12});this.noise({duration:.21,gain:.046*k,frequency:3300,to:820,Q:3.6});break;
        case'miaThrowRelease':if(variant){this.tone({frequency:980,to:310,duration:.13,gain:.07*k,type:'square'});this.tone({frequency:620,to:190,duration:.18,gain:.05*k,delay:.035,type:'triangle'});this.noise({duration:.2,gain:.068*k,frequency:3800,to:640,Q:3.2})}else{this.noise({duration:.16,gain:.08*k,frequency:2200,to:520,Q:1.8});this.tone({frequency:420,to:185,duration:.17,gain:.055*k,type:'sawtooth'});this.tone({frequency:1180,to:1540,duration:.07,gain:.035*k,delay:.015,type:'triangle'})}break;
  ...
  window.NYAN_BIOME_AMBIENCE_PROFILES=BIOME_AMBIENCE_PROFILES;
})();
